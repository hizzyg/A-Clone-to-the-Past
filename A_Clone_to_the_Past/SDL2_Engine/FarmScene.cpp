...
FarmScene::FarmScene(Engine * _pEngine)
	: Scene(_pEngine)
{
}
...
void FarmScene::Render(Renderer * _pRenderer)
{
...
void FarmScene::Update(float _deltaTime)
{
...
void FarmScene::Load(Renderer * _pRenderer)
{
	// save renderer
	...
	m_pWorld = new FarmWorld(_pRenderer, GetAssetPath("Texture/World/T_Atlas_Farm.png", 5).c_str(),
		new Rect(), nullptr);
	m_pWorld->LoadLayer1(_pRenderer, GetAssetPath("Texture/World/T_Farm_Layer1_Bitmap.bmp", 5).c_str());
	m_pWorld->LoadLayer2(_pRenderer, GetAssetPath("Texture/World/T_Farm_Layer2_Bitmap.bmp", 5).c_str());
	m_pWorld->LoadLayer3(_pRenderer, GetAssetPath("Texture/World/T_Farm_Layer3_Bitmap.bmp", 5).c_str());
	m_pWorld->LoadLayer4(_pRenderer, GetAssetPath("Texture/World/T_Farm_Layer4_Bitmap.bmp", 5).c_str());
	m_pWorld->LoadLayer5(_pRenderer, GetAssetPath("Texture/World/T_Farm_Layer5_Bitmap.bmp", 5).c_str());
	...
	m_pBackgroundMusic = new Music(F_MUSIC_FARM);
	m_pBackgroundMusic->Play(true);

	// scene change trigger
	SceneChanger* forest = new SceneChanger(_pRenderer, nullptr, new Rect(1984, 1408, 64, 320),
		nullptr, new ForestScene(m_pEngine), new Point(192, 7616));

	forest->SetCollider(new Collider(new Rect(64, 320), forest->GetRect(), ECollisionType::TRIGGER));

	m_pMoveEntities.push_back(forest);
}

void FarmScene::Unload()
{
...
// Nils Jungjohann
void FarmScene::CreateEntities(Renderer* _pRenderer)
{
	// Create all Entities from textfile
	std::ifstream file;
	std::string text = "";
	...
	file.open(GetAssetPath("Save/Save1/FarmScene.txt", 5).c_str());
	if (file.is_open())
		text = LoadStringFromFile(GetAssetPath("Save/Save1/FarmScene.txt", 5).c_str());
	else
		text = LoadStringFromFile(GetAssetPath("Save/Templates/FarmScene.txt", 5).c_str());
...
void FarmScene::CheckCollision(float _deltaTime)
{
	// bool if player dead
