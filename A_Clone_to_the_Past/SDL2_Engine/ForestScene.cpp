...
ForestScene::ForestScene(Engine * _pEngine)
	: Scene(_pEngine)
{
}
...
void ForestScene::Render(Renderer * _pRenderer)
{
...
void ForestScene::Update(float _deltaTime)
{
...
void ForestScene::Load(Renderer * _pRenderer)
{
	// save renderer
	...
	// Christial Thielsch
	// load world
	m_pWorld = new ForestWorld(_pRenderer, GetAssetPath("Texture/World/T_Atlas_Forest.png", 5).c_str(),
		new Rect(), nullptr);
	m_pWorld->LoadLayer1(_pRenderer, GetAssetPath("Texture/World/T_Forest_Layer1_Bitmap.bmp", 5).c_str());
	m_pWorld->LoadLayer2(_pRenderer, GetAssetPath("Texture/World/T_Forest_Layer2_Bitmap.bmp", 5).c_str());
	m_pWorld->LoadLayer3(_pRenderer, GetAssetPath("Texture/World/T_Forest_Layer3_Bitmap.bmp", 5).c_str());
	...
	m_pBackgroundMusic = new Music(F_MUSIC_FOREST);
	m_pBackgroundMusic->Play(true);

	// scene change trigger farm
	SceneChanger* farm = new SceneChanger(_pRenderer, nullptr, new Rect(0, 7488, 64, 320),
		nullptr, new FarmScene(m_pEngine), new Point(1856, 1536));

	farm->SetCollider(new Collider(new Rect(64, 320), farm->GetRect(), ECollisionType::TRIGGER));

	m_pMoveEntities.push_back(farm);

	// scene change trigger town
	SceneChanger* town = new SceneChanger(_pRenderer, nullptr, new Rect(0, 704, 64, 192),
		nullptr, new TownScene(m_pEngine), new Point(1856, 384));

	town->SetCollider(new Collider(new Rect(64, 192), town->GetRect(), ECollisionType::TRIGGER));

	m_pMoveEntities.push_back(town);
}
...
// Nils Jungjohann
void ForestScene::CreateEntities(Renderer* _pRenderer)
{
	// Create all Entities from textfile
	std::ifstream file;
	std::string text = "";
	...
	file.open(GetAssetPath("Save/Save1/ForestScene.txt", 5).c_str());
	if (file.is_open())
		text = LoadStringFromFile(GetAssetPath("Save/Save1/ForestScene.txt", 5).c_str());
	else
		text = LoadStringFromFile(GetAssetPath("Save/Templates/ForestScene.txt", 5).c_str());
...
void ForestScene::CheckCollision(float _deltaTime)
{
	// bool if player dead
